...
2. Self-contradiction: prove the allowed paths and forbidden actions still permit every mandatory checklist item, especially global Attention linkage, secure program approval, mobile navigation, detail data, cross-machine activation reporting, and one complete vertical candidate.
...
5. Acceptance: identify criteria that are impossible, ambiguous, non-finite, or require a source the system does not have. Pay special attention to live route selection, the single authoritative ACP source host, program approval snapshots, cancellation, intentional release pins, and terminal session linkage.
