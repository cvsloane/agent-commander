...
| homelinux | OpenAI Codex CLI 0.145.0 via login PATH | `gpt-5.6-sol` | yes | Ample, operator-reported shared subscription | Subscription window; exact reset not exposed | 2026-07-22T12:19:13-04:00 | Chris operator report plus verified `~/.local/bin/codex` 0.145.0 and successful remote login-shell `ROUTE_OK` probe | `ssh homelinux 'bash -lic '\''codex exec -m gpt-5.6-sol -c model_reasoning_effort="xhigh" -s read-only --ephemeral -C /home/cvsloane/dev/agent-command "Respond exactly ROUTE_OK and take no other action."'\'''` |
...
| Reviewer - critical | homelinux | OpenAI Codex CLI 0.145.0 via login PATH | `gpt-5.6-sol` | xhigh | HOLD at UI-reported 15% remaining | heavisidelinux / `gpt-5.6-sol` / xhigh fresh context | HOLD and escalate | Auth/credential candidate cannot receive a fresh-context review | Independent fresh context and clean worktree; receives checklist and frozen evidence, not builder conversation |
