---
lane: FW2-CONTRACTS
frozen_sha: 40bbd14d231cf54788a0377241c3e0cf6be1b9c2
attempt: 1
state: frozen
gates:
  lint: pass
...
proof:
  ...
  - "pnpm test:ci → 75 test files and 360 tests passed across all four workspaces"
  ...
  - "pnpm --filter @agent-command/control-plane test → 49 test files and 192 tests passed, including topology relay, unknown-envelope tolerance, tmux command policy, scrollback, fleet aggregation, and rename synchronization"
  ...
  - "git diff --exit-code refactor/frontend-command-center...40bbd14d231cf54788a0377241c3e0cf6be1b9c2 -- tests/fixtures/protocol → empty; every frozen JSON fixture is unchanged"
  - "git diff --name-only refactor/frontend-command-center...40bbd14d231cf54788a0377241c3e0cf6be1b9c2 -- apps/dashboard migrations agents deploy → empty"
...
- The complete required gate sequence passed at the frozen implementation SHA. `test:ci` retains pre-existing, non-failing notification mock diagnostics, and dashboard smoke retains pre-existing mocked-response validation warnings for omitted `capture_hash`/`groups` fields; both suites completed green.
...
FW2-CONTRACTS FROZEN 40bbd14d231cf54788a0377241c3e0cf6be1b9c2
