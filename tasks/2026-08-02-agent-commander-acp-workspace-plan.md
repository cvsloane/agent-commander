...
- The current local `030b4095cf8d9473f7de0a4521cf0b97cf1fe9b9` project-status commit and the Human Owner correction in `tasks/lessons.md` are preserved through integration. Production ancestry starts at `b99a33cd202af2de69ac555d68a7e4a12fef0d9b`; ACP policy selected local worktree base `3dccd71f92eb529b9286060f1ed5c2f2965a996c` after the approved control commits.
...
- Runtime preconditions: before production action proof, the AI Lead writes only `CODING_DISPATCH_ALLOWED_CLI_APPROVERS=chris` into the canonical agentd/Hermes runtime, maps control-plane `ACP_SOURCE_HOST_ID` to the authenticated heavisidelinux UUID, and maps `ACP_APPROVER_USER_ID` to the exact lowercase authenticated Human Owner UUID. Service changes occur only through the already approved final deployment. Proof covers refusal when either approver identity is absent/mismatched and unavailable ACP state when the source host identity is absent/invalid. Rollback removes the three non-secret mappings and restarts the affected services.
