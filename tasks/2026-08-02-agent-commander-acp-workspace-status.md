...
- Current phase: W1 ACP dispatch
- Overall state: running
- Last updated: 2026-08-03T01:32:19Z
...
| W1 full ACP workspace | Luna Builder | ready to dispatch | One Agent Commander vertical candidate | Three bounded Opus brief passes found and drove direct source-contract corrections; AI Lead verified the final facts on heavisidelinux activated release `817a8ee7` | Final capability/quota/route recheck | Dispatch once through ACP, then inspect the real worktree |
...
1. Recheck capability, quota, and scoped Luna resolution.
2. Dispatch the frozen W1 brief through ACP with Luna Max and the existing Go build verifier only.
3. Monitor the real ACP task and inspect its worktree/diff rather than summary metadata.
4. Integrate the frozen candidate once and run only the affected production builds.
