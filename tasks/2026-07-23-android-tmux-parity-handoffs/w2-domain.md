...
frozen_sha: 59eec2d43ff82902d8e70932d2b82a1887dab776
acceptance:
  - NAV-1
  - NAV-2
  - NAV-3
...
gate:
  ...
  status: passed
  results:
    - "25/25 debug unit tests passed"
    - "25/25 release unit tests passed"
    - "Android lint passed"
    - "Unsigned release assembly passed"
    - "app-release-unsigned.apk: 2,272,190 bytes; SHA-256 05b886f907374abcbb97a4d147c22fefc172b519bd8fd61c28123c30d993d47e"
blockers: []
---
...
- `TmuxRoster` deterministically models host → tmux session → window → pane, supports stable multi-term search and host/provider/status/online filters, validates a restored target against all stable coordinates, and enriches the tree with live active/zoom/bell/activity/size/path state without replacing durable pane metadata.
...
- `TmuxRosterTest` (4): deterministic hierarchy, search/filter/snapshot preservation, restored-target validation, live-topology enrichment.
...
- **NAV-1 / NAV-4:** Global multi-host roster consumption, deterministic hierarchy/search/filter helpers, host presence, pane status/activity/snapshot context, and live topology enrichment are source-backed. Phone presentation remains W3-owned.
- **NAV-2:** Typed live topology and command-result truth plus the additive viewer-state request are ready for W1/W3 switching integration. Transactional UI wiring remains outside W2.
...
- The documented roster currently omits snapshots; the Android parser preserves `latest_snapshot` when supplied and never discards existing pane metadata while applying live topology.
